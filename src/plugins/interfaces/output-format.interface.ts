...
export interface IOutputFormatProvider extends IPlugin {
  ...
  generateFromTemplate(templateId: string, data: any, options?: Partial<IOutputOptions>): Promise<IOutputResult>;

  getTemplates(): ITemplate[];

  registerTemplate(template: ITemplate): Promise<void>;

  updateTemplate(templateId: string, template: Partial<ITemplate>): Promise<void>;

  deleteTemplate(templateId: string): Promise<boolean>;

  previewTemplate(templateId: string, data: any): Promise<string>;

  convertFormat(inputPath: string, outputFormat: string, options?: Partial<IOutputOptions>): Promise<IOutputResult>;

  mergeOutputs(inputPaths: string[], outputPath: string, options?: Partial<IOutputOptions>): Promise<IOutputResult>;
