...
app.get("/api/debug", (req, res) => {
  try {
    ...
    const pluginStatus = plugins.map(plugin => ({
    ...
    res.json({
      ...
      researchService: {
        dataProviders: researchService['dataProviders']?.length || 0,
        analyticsProviders: researchService['analyticsProviders']?.length || 0,
        aiProviders: researchService['aiProviders']?.length || 0,
        outputProviders: researchService['outputProviders']?.length || 0,
      }
    });
  } catch (error) {
    res.status(500).json({
...
app.post("/api/test", async (req, res) => {
  try {
    ...
    console.log("🧪 Starting test research with params:", params);

    const result = {
      marketTrends: `Basic market trends analysis for ${params.industry} in ${params.region} region for ${params.timeFrame}. This is a test response.`,
      sources: [],
      chartConfigs: [
        {
          type: 'bar',
          data: {
            labels: ['Q1', 'Q2', 'Q3', 'Q4'],
            datasets: [{ label: 'Test Data', data: [10, 15, 12, 18] }]
          },
          options: { animation: { duration: 0 } }
        }
      ],
      htmlReport: '<h1>Test Report</h1><p>This is a test report.</p>',
      pdfPath: `test-report-${Date.now()}.pdf`,
    };

    const path = await import('path');
    const fileName = result.pdfPath ? path.basename(result.pdfPath) : 'test-report.pdf';
    const pdfUrl = `http://localhost:3000/reports/${fileName}`;
